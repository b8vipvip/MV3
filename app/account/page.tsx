...
export default function AccountPage() {
  ...
  return <section className="shell page-section"><div className="page-hero narrow"><span className="eyebrow">MV3 IDENTITY</span><h1>{cn ? '一个账户，连接整个码位3。' : locale === 'ja' ? '1つのアカウントで、MV3 全体へ。' : 'One account across MV3.'}</h1><p>{cn ? '当前码位3社区账户已经作为统一身份基础启用；后续会通过显式绑定和 OIDC 分阶段连接 GPTWork、chat2api、QNBot、FDEX 与 Aipany。' : locale === 'ja' ? 'MV3 Community アカウントを共通ID基盤として開始し、今後は明示的なリンクと OIDC で各製品へ段階的に接続します。' : 'The MV3 Community account is now the identity foundation. Products will connect in stages through explicit account linking and OIDC.'}</p></div>{!databaseReady && <div className="notice warning">{cn ? '当前部署尚未配置数据库连接。为兼容现有部署，运行时变量仍使用 CN2_DATABASE_URL；完成 PostgreSQL 配置并运行 npm run db:init 后即可启用注册、登录和论坛。' : 'This deployment has not configured the database connection yet. For deployment compatibility, the runtime variable remains CN2_DATABASE_URL. Configure PostgreSQL and run npm run db:init to enable accounts and the forum.'}</div>}{user ? <div className="identity-card"><div><span className="status-pill">SIGNED IN</span><h2>{user.displayName}</h2><p>{user.email}</p><button className="button secondary" onClick={logout}>{cn ? '退出登录' : 'Sign out'}</button></div><div className="identity-flow"><span>MV3 Identity</span><i>→</i><span>Community ✓</span><span>GPTWork · next</span><span>chat2api · next</span><span>QNBot · next</span><span>FDEX · next</span><span>Aipany · next</span></div></div> : <div className="auth-layout"><div className="auth-card"><div className="auth-tabs"><button className={mode === 'login' ? 'active' : ''} onClick={() => setMode('login')}>{cn ? '登录' : 'Sign in'}</button><button className={mode === 'register' ? 'active' : ''} onClick={() => setMode('register')}>{cn ? '注册' : 'Register'}</button></div><form onSubmit={submit}>{mode === 'register' && <label>{cn ? '显示名称' : 'Display name'}<input name="displayName" minLength={2} maxLength={64} required /></label>}<label>{cn ? '邮箱' : 'Email'}<input name="email" type="email" required autoComplete="email" /></label><label>{cn ? '密码（至少 10 位）' : 'Password (10+ characters)'}<input name="password" type="password" minLength={10} maxLength={200} required autoComplete={mode === 'login' ? 'current-password' : 'new-password'} /></label>{status && <span className="form-error">{status}</span>}<button className="button primary" type="submit">{mode === 'login' ? (cn ? '登录码位3' : 'Sign in to MV3') : (cn ? '创建码位3账户' : 'Create MV3 account')}</button></form></div><div className="auth-copy"><span className="eyebrow">ACCOUNT FOUNDATION</span><h2>{cn ? '先统一身份，再逐步统一登录。' : 'Unify identity first, then sign-in.'}</h2><p>{cn ? '不会因为邮箱相同就自动合并旧产品账号。每个已有账号都需要用户显式绑定与重新验证，避免错误合并和权限串联。' : 'Existing product accounts are never merged merely because emails match. Linking requires explicit confirmation and re-verification.'}</p></div></div>}</section>;
}
