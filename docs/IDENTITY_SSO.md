# CN2 Identity 与统一账户路线 / CN2 Identity & Shared Account Roadmap

## 中文

目标：最终让一个 CN2 账户登录社区以及 GPTWork、chat2api、QNBot、FDEX、Aipany 等 CN2 产品，同时保持各产品业务数据、设备、权益和权限边界清晰。

### 原则

1. **CN2 Identity 是身份源，不是所有产品数据库的合并数据库。**
2. 各产品使用稳定的 `cn2_user_id` 作为跨产品身份映射，同时保留自己的业务主键。
...
- 建立 CN2 Identity 数据模型和 OIDC endpoints；
- 社区首先使用 CN2 Identity；
...
- 用户登录已有 GPTWork / FDEX 等账号后主动绑定 CN2 账户；
...
- 绑定账号前要求显式确认；
- 关键操作支持重新认证；
- 全局注销可撤销 CN2 Session，但产品侧高风险长会话仍需自己的撤销/过期策略；
- 任何迁移都必须有可回滚映射与审计记录。
...
Goal: one CN2 account should eventually sign into the community and CN2 products such as GPTWork, chat2api, QNBot, FDEX and Aipany, while product business data, devices, entitlements and authorization remain clearly separated.

CN2 Identity is the source of identity, not a merged database for every product. Products map a stable `cn2_user_id` to their own business user keys. Use standards-compatible OAuth 2.1 / OpenID Connect Authorization Code + PKCE flows, rotating refresh tokens, device/session records and explicit revocation.
